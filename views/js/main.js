...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    var ranContainer = document.getElementsByClassName("randomPizzaContainer");
    var dx = determineDx(ranContainer[0], size);
    var newwidth = (ranContainer[0].offsetWidth + dx) + 'px';

    for (var i = 0; i < ranContainer.length; i++) {
      ranContainer[i].style.width = newwidth;
    }
...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin((docTop / 1250) + (i % 5));
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  console.log("totalPizzas: "+ totalPizzas);
